...
SIRV.generic <- function(t,
  ...
  ## susceptibles
  dS <- as.numeric(params$nat_wane*R + as.numeric(params$V_step*V.states.vector[params$n.comps.V]) +params$mig_in*(N*(1-params$foreign_infection)) +params$birth_death_rate*N -lambda*S -params$mig_out*S -params$birth_death_rate*S -revax_out$mass_S_rate -revax_out$birth - revax_out$migrant -revax_out$routine_S_rate)
