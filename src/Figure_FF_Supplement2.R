#### Figure FF ####
# Assuming a non-waning Shanchol vaccine

setwd("/Users/peakcm/Dropbox/Cholera Amanda/cholera_waning")

#### Load workspace ####
load(file = "src/Figure_FF_Supplement2.RData")
...
VE <- rep(max(VE), length(VE)) # Non-waning, perfect Shanchol
...
vac_mass_frac_conditions <- c(1, 0.7)     # This also applies to the initial mass coverage for "Mass" and "Mass_Maintain"
# vac_mass_frac_conditions <- c(1, 0.4)     # When using perfect vaccine

vac_mass_freq_conditions <- c(365, 365*2) # Applies only to "Mass"
vac_routine_count_conditions <- c(8, 10, 12) # Applies to "Routine" and "Mass_Maintain"
...
fig_FF_df_melt[fig_FF_df_melt$strategy == "Mass-then-Maintain" & fig_FF_df_melt$vac_mass_frac_condition == 0.7, "keep"] <- 1
...
fig_FF_df_bars$y = NA
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass-then-Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.7 & fig_FF_df_bars$vac_routine_count_condition == 8, "y"] <- 0.5
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass-then-Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.7 & fig_FF_df_bars$vac_routine_count_condition == 10, "y"] <- 0.4
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass-then-Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.7 & fig_FF_df_bars$vac_routine_count_condition == 12, "y"] <- 0.3

fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass Vaccination" & fig_FF_df_bars$vac_mass_frac_condition == 1 & fig_FF_df_bars$vac_mass_freq_condition == 360, "y"] <- 0.5
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass Vaccination" & fig_FF_df_bars$vac_mass_frac_condition == 1 & fig_FF_df_bars$vac_mass_freq_condition == 730, "y"] <- 0.4

fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 8, "y"] <- 0.5
fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 10, "y"] <- 0.4
fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 12, "y"] <- 0.3
...
ggsave(file = "figures/Figure_FF_Supplement2.pdf", width = 5, height = 3, units = "in")
...
#### Save workspace ####
save.image(file = "src/Figure_FF_Supplement2.RData")
