...
jj <- ggplot(fig_AA_df_melt[fig_AA_df_melt$birth_death_rate_condition_name == "None" & fig_AA_df_melt$VE_condition_name %in% c("kOCV", "Perfect Vaccine"),], aes(x = times/365, y = Re, linetype = mig_condition_name)) + geom_line() + facet_grid(.~VE_condition_name) +  ylab("Proportion Susceptible X(t)") + theme_bw()  + scale_linetype_manual(name = "In/Out Migration Rate", values = c("solid", "longdash", "dotted")) + theme(text = element_text(size=10), legend.text=element_text(size=10), legend.title=element_text(size=10)) + scale_x_continuous(labels = NULL, breaks = c(0,2,4,6,8,10), name = NULL) + theme(panel.grid.major = element_blank()) + theme(panel.grid.minor = element_blank())
