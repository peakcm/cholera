...
jj_3 <- ggplot(fig_BB_df_melt_jj_3[fig_BB_df_melt_jj_3$VE_condition_name %in% c("kOCV", "Perfect Vaccine"),], aes(x = times/365, y = prob_outbreak_10, linetype = mig_condition_name, color = R0_condition_name)) + geom_line() + facet_grid(.~VE_condition_name) + xlab("Years since Mass Vaccination") + ylab("Probability of an Outbreak\nafter Cholera Introduction") + theme_bw() + scale_color_discrete(name = expression(paste("Basic Reproductive Number ",(R[0])))) + scale_linetype_manual(name = "In/Out Migration Rate",values =  c("solid", "longdash")) + ylim(0,1) + theme(text = element_text(size=10), legend.text=element_text(size=10), legend.title=element_text(size=10)) + guides(lty = FALSE, color = FALSE) + theme(panel.grid.major = element_blank()) + theme(panel.grid.minor = element_blank()) + scale_x_continuous(name = "Years since Mass Vaccination", breaks = c(0,2,4,6,8,10)) + theme(strip.background = element_blank(),strip.text.x = element_blank())
