...
#### Load workspace ####
load(file = "src/Figure_FF_Supplement1.RData")
...
vac_mass_freq_conditions <- c(365*2, 365*4) # Applies only to "Mass"
vac_routine_count_conditions <- c(4.5, 7, 8) # Applies to "Routine" and "Mass_Maintain"

# mig_conditions <- c("low" = 1/(365*20), "moderate" = 1/(365*4.3), "high" = 1/(365*2)) # Applies to all
mig_conditions <- c("low" = 1/(365*20))
...
for (group in fig_FF_df_bars_summary$group){
  DHI <- sum(fig_FF_df_bars[fig_FF_df_bars$group == group, "DHI"])
  fig_FF_df_bars_summary[fig_FF_df_bars_summary$group == group, ] <- c(group, DHI, fig_FF_df_bars[fig_FF_df_bars$group == group, c("strategy", "vac_mass_frac_condition", "vac_mass_freq_condition", "vac_routine_count_condition")]) 
  
  fig_FF_df_bars[fig_FF_df_bars$group == group, "DHI_group"][length(fig_FF_df_bars[fig_FF_df_bars$group == group, "DHI_group"])] <- paste(round(DHI/365,1), "years")
  cat(".")
}
...
fig_FF_df_melt[fig_FF_df_melt$strategy == "Mass Vaccination" & fig_FF_df_melt$vac_mass_frac_condition == 0.75, "keep"] <- 1
...
fig_FF_df_bars$y = NA
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass and Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.75 & fig_FF_df_bars$vac_routine_count_condition == 4.5, "y"] <- 0.5
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass and Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.75 & fig_FF_df_bars$vac_routine_count_condition == 7, "y"] <- 0.4
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass and Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.75 & fig_FF_df_bars$vac_routine_count_condition == 8, "y"] <- 0.3
...
fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass Vaccination" & fig_FF_df_bars$vac_mass_frac_condition == 1 & fig_FF_df_bars$vac_mass_freq_condition == 1460, "y"] <- 0.4

fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 4.5 , "y"] <- 0.5
fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 7 , "y"] <- 0.4
fig_FF_df_bars[fig_FF_df_bars$strategy == "Routine Vaccination" & fig_FF_df_bars$vac_routine_count_condition == 8 , "y"] <- 0.3
...
ggplot(fig_FF_df_melt[fig_FF_df_melt$keep == 1,], aes(x = times/365, y = Re, color = factor(vac_routine_count_condition), linetype = factor(1-vac_mass_freq_condition))) + geom_hline(yintercept = 1, col = "darkgrey") + geom_line() + facet_grid(strategy ~.) + xlab("Years") + ylab("Effective Reproductive Number") + theme_bw() + ylim(0,2) + theme(text = element_text(size=6), legend.text=element_text(size=6), legend.title=element_text(size=6)) + scale_color_discrete(name = "Routine Vaccine\nCourses per Day") + scale_linetype_manual("Mass Vaccination Frequency", values = c("dotted", "longdash", "solid"), labels = c("2 Years", "1 Year", "N/A")) + guides(linetype = guide_legend(order = 1), color = guide_legend(order = 2)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  ...
  geom_text(data = fig_FF_df_bars, aes(x = t_end/365+1, y = y, label = DHI_group), size = 1.5, alpha = 0.5)

ggsave(file = "figures/Figure_FF_Supplement1.pdf", width = 5, height = 3, units = "in")

# #### Plot Re over time (best of each) ####
# fig_FF_df_melt$best <- 0
# fig_FF_df_melt[fig_FF_df_melt$strategy == "Mass and Maintain" & fig_FF_df_melt$vac_mass_frac_condition == 0.75 & fig_FF_df_melt$vac_routine_count_condition == 12, "best"] <- 1
...
# fig_FF_df_bars[fig_FF_df_bars$strategy == "Mass and Maintain" & fig_FF_df_bars$vac_mass_frac_condition == 0.75 & fig_FF_df_bars$vac_routine_count_condition == 12, "y"] = 0.5
...
# #### Plot Re over time ####
# ggplot(fig_FF_df_melt[fig_FF_df_melt$vac_mass_freq_condition != 730 & fig_FF_df_melt$vac_mass_frac_condition %in% c(0, 0.75, 1),], aes(x = times/365, y = Re, color = factor(vac_routine_count_condition), linetype = factor(vac_mass_frac_condition ))) + geom_hline(yintercept = 1, col = "darkgrey") + geom_line() + facet_grid(strategy ~.) + xlab("Years") + ylab("Effective Reproductive Number") + theme_bw() + ylim(0,2) + theme(text = element_text(size=6), legend.text=element_text(size=6), legend.title=element_text(size=6)) + scale_linetype_manual(name = "Mass Vaccination Coverage", values = c("dotted", "longdash", "solid")) + scale_color_discrete(name = "Routine Doses per Day") 
# 
...
#### Save workspace ####
save.image(file = "src/Figure_FF_Supplement1.RData")
